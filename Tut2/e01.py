...
with tf.Session() as sess:
    ...
    print("x:{0} y:{1} out:{2}".format(x,y,out))
###############################################################################
...
x = tf.random_uniform([],minval=-1)
y = tf.random_uniform([],minval=-1)
out = tf.case([(tf.less(x, y), lambda: tf.add(x, y)), (tf.greater(x, y), lambda: tf.subtract(x, y))], default=lambda:tf.constant(0.0))
...
x = tf.random_uniform([10,10])  # Empty array as shape creates a scalar.
...
out = tf.cond(tf.greater(0.0,tf.reduce_mean(x-y)), lambda: tf.reduce_mean(tf.square(x-y)), lambda: tf.reduce_sum(tf.abs(x-y)))
